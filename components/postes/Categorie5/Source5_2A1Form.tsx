...
import { FiCalendar, FiMapPin, FiFileText, FiTrash2 } from "react-icons/fi";
import { supabase } from "../../../lib/supabaseClient";
import { ReferenceSelect } from '../ReferenceSelect';

export type Source5_2A1Row = {
  description: string;
  date: string;
  site: string;
  product: string;
...
export interface Source5_2A1FormProps {
  rows: Source5_2A1Row[];
  setRows: React.Dispatch<React.SetStateAction<Source5_2A1Row[]>>;
...
const SOURCE_CODE = "5.2A1";
const POSTE_NUM   = 5;

// kg CO₂e per tonne of waste landfilled (IPCC 2006, Tier 1 defaults)
const WASTE_FACTORS: Record<string, number> = {
  "Déchets mixtes (enfouissement)":          587,
  "Plastique":                               100,
  "Métal (acier/aluminium)":                  10,
  "Papier / carton":                         630,
  "Verre":                                     5,
  "Matières organiques (biodégradables)":   1200,
  "Textiles":                                580,
  "Bois":                                    470,
  "Électronique (DEEE)":                     200,
};
const WASTE_OPTIONS = Object.keys(WASTE_FACTORS);

const EMPTY_ROW: Source5_2A1Row = {
  description: "", date: "", site: "", product: "", reference: "",
  wasteType: WASTE_OPTIONS[0], qtySold: "", weightKgPerUnit: "",
};
...
const calcRow = (row: Source5_2A1Row) => {
  const qty      = toNum(row.qtySold);
  const kg       = toNum(row.weightKgPerUnit);
  const factor   = WASTE_FACTORS[row.wasteType] ?? 587; // kg CO₂e / tonne
  const tonnesWaste = (qty * kg) / 1000;
  const kgCO2e   = tonnesWaste * factor;
  return { co2: kgCO2e * 1000, total: kgCO2e * 1000, tco2e: kgCO2e / 1000 };
};

export function Source5_2A1Form({ rows, setRows, posteSourceId, userId, gesResults = [], setGesResults }: Source5_2A1FormProps) {
  ...
  const triggerSave = (next: Source5_2A1Row[]) => {
  ...
  const updateRow = (idx: number, key: keyof Source5_2A1Row, val: string) => {
    ...
    <Box bg={FIGMA.bg} borderRadius={FIGMA.r} p={{ base: 4, md: 6 }} boxShadow={FIGMA.cardShadow} animation={`${fadeInUp} 0.4s ease`}>
      <Flex justify="space-between" align="center" mb={5}>
        <VStack align="start" spacing={0}>
          <Heading size="md" color={FIGMA.green}>5.2A1 — Mise en décharge des produits vendus</Heading>
          <Text fontSize="sm" color={FIGMA.muted}>Émissions CH₄ liées à l&apos;enfouissement des produits en fin de vie</Text>
        </VStack>
        <HStack spacing={2}>
            ...
            <Grid templateColumns={{ base: "1fr", md: "repeat(3,1fr)" }} gap={3}>
              <GridItem colSpan={{ base: 1, md: 3 }}>
                ...
                <Input size="sm" borderRadius={FIGMA.inputR} value={row.description}
                  onChange={e => updateRow(idx, "description", e.target.value)} placeholder="ex. Emballage plastique, composants électroniques…" />
              </GridItem>

              <GridItem colSpan={{ base: 1, md: 2 }}>
                <Text fontSize="xs" color={FIGMA.muted} mb={1}><Icon as={FiTrash2} mr={1} />Type de déchet (matériau dominant)</Text>
                <Select size="sm" borderRadius={FIGMA.inputR} value={row.wasteType}
                  onChange={e => updateRow(idx, "wasteType", e.target.value)}>
                  {WASTE_OPTIONS.map(w => <option key={w}>{w}</option>)}
                </Select>
              </GridItem>
            ...
            {toNum(row.qtySold) > 0 && toNum(row.weightKgPerUnit) > 0 && (
              <Flex mt={3} p={2} bg={FIGMA.greenSoft} borderRadius="10px" align="center" justify="space-between">
                <Text fontSize="xs" color={FIGMA.green} fontWeight="600">
                  {((toNum(row.qtySold) * toNum(row.weightKgPerUnit)) / 1000).toFixed(3)} t déchets · {WASTE_FACTORS[row.wasteType] ?? 587} kg CO₂e/t
                </Text>
...
export default Source5_2A1Form;
