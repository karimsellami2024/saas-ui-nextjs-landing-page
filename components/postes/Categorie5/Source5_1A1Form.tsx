...
export interface Source5_1A1FormProps {
  rows: Source5_1A1Row[];
  setRows: React.Dispatch<React.SetStateAction<Source5_1A1Row[]>>;
...
const SOURCE_CODE = "5.1A1";
const POSTE_NUM   = 5;

// gCO₂e per kWh (same provincial grid factors as 4.1C1)
...
const EMPTY_ROW: Source5_1A1Row = {
  description: "", date: "", site: "", product: "", reference: "",
  province: "Québec", qtySold: "", kwhPerUnitPerYear: "", yearsOfUse: "1",
};
...
const calcRow = (row: Source5_1A1Row) => {
  const qty    = toNum(row.qtySold);
  const kwh    = toNum(row.kwhPerUnitPerYear);
  const years  = toNum(row.yearsOfUse) || 1;
  const factor = GRID_FACTORS[row.province] ?? 1.3; // gCO₂e/kWh
  const gCO2e  = qty * kwh * years * factor;
  return { co2: gCO2e, total: gCO2e, tco2e: gCO2e / 1_000_000 };
};

export function Source5_1A1Form({ rows, setRows, posteSourceId, userId, gesResults = [], setGesResults }: Source5_1A1FormProps) {
  ...
  const triggerSave = (next: Source5_1A1Row[]) => {
  ...
  const updateRow = (idx: number, key: keyof Source5_1A1Row, val: string) => {
    ...
    <Box bg={FIGMA.bg} borderRadius={FIGMA.r} p={{ base: 4, md: 6 }} boxShadow={FIGMA.cardShadow} animation={`${fadeInUp} 0.4s ease`}>
      <Flex justify="space-between" align="center" mb={5}>
        <VStack align="start" spacing={0}>
          <Heading size="md" color={FIGMA.green}>5.1A1 — Produits vendus consommant de l&apos;électricité</Heading>
          <Text fontSize="sm" color={FIGMA.muted}>Émissions liées à l&apos;utilisation électrique des produits pendant leur durée de vie</Text>
        </VStack>
        <HStack spacing={2}>
      ...
      <VStack spacing={4} align="stretch">
        {rows.map((row, idx) => (
          <Box key={idx} bg="white" borderRadius={FIGMA.r} p={4}
            ...
            <Grid templateColumns={{ base: "1fr", md: "repeat(3,1fr)" }} gap={3}>
              <GridItem colSpan={{ base: 1, md: 3 }}>
                ...
                <Input size="sm" borderRadius={FIGMA.inputR} value={row.description}
                  onChange={e => updateRow(idx, "description", e.target.value)} placeholder="ex. Lave-vaisselle modèle XL…" />
              </GridItem>

              <GridItem colSpan={{ base: 1, md: 2 }}>
                <Text fontSize="xs" color={FIGMA.muted} mb={1}><Icon as={FiZap} mr={1} />Province d&apos;utilisation (réseau électrique)</Text>
              ...
              <GridItem>
                <Text fontSize="xs" color={FIGMA.muted} mb={1}>kWh / unité / an</Text>
                <Input size="sm" borderRadius={FIGMA.inputR} type="number" min={0}
                  value={row.kwhPerUnitPerYear} onChange={e => updateRow(idx, "kwhPerUnitPerYear", e.target.value)} placeholder="0" />
              </GridItem>
            ...
            {toNum(row.qtySold) > 0 && toNum(row.kwhPerUnitPerYear) > 0 && (
              <Flex mt={3} p={2} bg={FIGMA.greenSoft} borderRadius="10px" align="center" justify="space-between">
                <Text fontSize="xs" color={FIGMA.green} fontWeight="600">
                  {GRID_FACTORS[row.province] ?? 1.3} gCO₂e/kWh · {toNum(row.yearsOfUse) || 1} an(s)
                </Text>
...
export default Source5_1A1Form;
