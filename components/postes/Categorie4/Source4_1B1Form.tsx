...
import { FiCalendar, FiMapPin, FiFileText, FiWifi } from "react-icons/fi";
import { supabase } from "../../../lib/supabaseClient";
import { ReferenceSelect } from '../ReferenceSelect';

export type Source4_1B1Row = {
  description: string;
  date: string;
  site: string;
  product: string;
  reference: string;
  dataTransferredGB: string;
};
...
export interface Source4_1B1FormProps {
  rows: Source4_1B1Row[];
  setRows: React.Dispatch<React.SetStateAction<Source4_1B1Row[]>>;
...
const SOURCE_CODE    = "4.1B1";
const POSTE_NUM      = 4;
const KG_CO2E_PER_GB = 0.000272;

const EMPTY_ROW: Source4_1B1Row = {
  description:"", date:"", site:"", product:"", reference:"", dataTransferredGB:"",
};
...
const calcRow = (row: Source4_1B1Row) => {
  const gb     = toNum(row.dataTransferredGB);
  const kgCO2e = gb * KG_CO2E_PER_GB;
  return { co2: kgCO2e * 1000, total: kgCO2e * 1000, tco2e: kgCO2e / 1000 };
};

export function Source4_1B1Form({ rows, setRows, posteSourceId, userId, bilanId, gesResults=[], setGesResults }: Source4_1B1FormProps) {
  ...
  const triggerSave = (next: Source4_1B1Row[]) => {
  ...
  const updateRow = (idx: number, key: keyof Source4_1B1Row, val: string) => {
    ...
    <Box bg={FIGMA.bg} borderRadius={FIGMA.r} p={{ base:4, md:6 }} boxShadow={FIGMA.cardShadow} animation={`${fadeInUp} 0.4s ease`}>
      <Flex justify="space-between" align="center" mb={5}>
        <VStack align="start" spacing={0}>
          <Heading size="md" color={FIGMA.green}>4.1B1 — Réseaux et transfert de données</Heading>
          <Text fontSize="sm" color={FIGMA.muted}>Émissions liées au transfert et stockage de données en réseau</Text>
        </VStack>
        <HStack spacing={2}>
        ...
        {rows.map((row, idx) => (
          <Box key={idx} bg="white" borderRadius={FIGMA.r} p={4}
            ...
            <Flex justify="space-between" align="center" mb={3}>
              <Badge bg={FIGMA.greenSoft} color={FIGMA.green} borderRadius="8px" px={3} py={1} fontSize="xs">Ligne {idx + 1}</Badge>
              <HStack spacing={1}>
            ...
            <Grid templateColumns={{ base:"1fr", md:"repeat(3,1fr)" }} gap={3}>
              <GridItem colSpan={{ base:1, md:3 }}>
                ...
                <Input size="sm" borderRadius={FIGMA.inputR} value={row.description}
                  onChange={e => updateRow(idx, "description", e.target.value)} placeholder="ex. Sauvegarde cloud mensuelle…" />
              </GridItem>

              <GridItem colSpan={{ base:1, md:2 }}>
                <Text fontSize="xs" color={FIGMA.muted} mb={1}><Icon as={FiWifi} mr={1}/>Données transférées (GB)</Text>
                <Input size="sm" borderRadius={FIGMA.inputR} type="number" min={0}
                  value={row.dataTransferredGB} onChange={e => updateRow(idx, "dataTransferredGB", e.target.value)} placeholder="0" />
              </GridItem>

              <GridItem>
                <Text fontSize="xs" color={FIGMA.muted} mb={1}>Facteur utilisé</Text>
                <Box px={3} py={2} bg={FIGMA.greenSoft} borderRadius="10px">
                  <Text fontSize="xs" color={FIGMA.green} fontWeight="600">{KG_CO2E_PER_GB} kg CO₂e / GB</Text>
                </Box>
              </GridItem>
            ...
            {toNum(row.dataTransferredGB) > 0 && (
              <Flex mt={3} p={2} bg={FIGMA.greenSoft} borderRadius="10px" align="center" justify="space-between">
                <Text fontSize="xs" color={FIGMA.green} fontWeight="600">Émissions estimées :</Text>
                <Text fontSize="xs" color={FIGMA.green} fontWeight="bold">
                  {calcRow(row).co2.toFixed(4)} g CO₂e &nbsp;|&nbsp; {calcRow(row).tco2e.toFixed(8)} t CO₂e
                </Text>
              </Flex>
            )}
          </Box>
      ...
      {rows.length > 0 && (
        <Grid mt={6} templateColumns={{ base:"1fr", md:"repeat(3,1fr)" }} gap={3}>
          {[
            { label:"CO₂ total [gCO₂e]", val: totals.co2.toFixed(4) },
            { label:"Total GES [gCO₂e]",  val: totals.total.toFixed(4) },
            { label:"Total GES [tCO₂e]",  val: totals.tco2e.toFixed(8) },
          ].map(card => (
...
export default Source4_1B1Form;
