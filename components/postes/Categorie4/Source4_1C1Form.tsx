...
import { FiCalendar, FiMapPin, FiFileText, FiServer } from "react-icons/fi";
import { supabase } from "../../../lib/supabaseClient";
import { ReferenceSelect } from '../ReferenceSelect';

export type Source4_1C1Row = {
  description: string;
  date: string;
  site: string;
  product: string;
...
export interface Source4_1C1FormProps {
  rows: Source4_1C1Row[];
  setRows: React.Dispatch<React.SetStateAction<Source4_1C1Row[]>>;
...
const GRID_FACTORS: Record<string, number> = {
  ...
  "Manitoba":                    2.0,
  "Nouveau-Brunswick":         290.0,
  "Nouvelle-Écosse":           670.0,
  "Île-du-Prince-Édouard":     100.0,
  "Saskatchewan":              500.0,
  "Alberta":                   490.0,
  "Terre-Neuve-et-Labrador":    14.0,
  "Territoires du Nord-Ouest": 420.0,
};
const PROVINCE_OPTIONS = Object.keys(GRID_FACTORS);

const EMPTY_ROW: Source4_1C1Row = {
  description:"", date:"", site:"", product:"", reference:"", province:"Québec", kwhConsumed:"",
};
...
const calcRow = (row: Source4_1C1Row) => {
  const kwh   = toNum(row.kwhConsumed);
  const factor = GRID_FACTORS[row.province] ?? 1.3;
  const gCO2e  = kwh * factor;
  return { co2: gCO2e, total: gCO2e, tco2e: gCO2e / 1_000_000 };
};

export function Source4_1C1Form({ rows, setRows, posteSourceId, userId, bilanId, gesResults=[], setGesResults }: Source4_1C1FormProps) {
  ...
  const triggerSave = (next: Source4_1C1Row[]) => {
  ...
  const updateRow = (idx: number, key: keyof Source4_1C1Row, val: string) => {
    ...
    <Box bg={FIGMA.bg} borderRadius={FIGMA.r} p={{ base:4, md:6 }} boxShadow={FIGMA.cardShadow} animation={`${fadeInUp} 0.4s ease`}>
      <Flex justify="space-between" align="center" mb={5}>
        <VStack align="start" spacing={0}>
          <Heading size="md" color={FIGMA.green}>4.1C1 — Salles de serveurs</Heading>
          <Text fontSize="sm" color={FIGMA.muted}>Émissions liées à la consommation électrique des serveurs</Text>
        </VStack>
        <HStack spacing={2}>
            ...
            <Grid templateColumns={{ base:"1fr", md:"repeat(3,1fr)" }} gap={3}>
              <GridItem colSpan={{ base:1, md:3 }}>
                ...
                <Input size="sm" borderRadius={FIGMA.inputR} value={row.description}
                  onChange={e => updateRow(idx, "description", e.target.value)} placeholder="ex. Salle serveur principal…" />
              </GridItem>

              <GridItem colSpan={{ base:1, md:2 }}>
                <Text fontSize="xs" color={FIGMA.muted} mb={1}><Icon as={FiServer} mr={1}/>Province / Réseau électrique</Text>
                <Select size="sm" borderRadius={FIGMA.inputR} value={row.province}
              ...
              <GridItem>
                <Text fontSize="xs" color={FIGMA.muted} mb={1}>Consommation (kWh)</Text>
                <Input size="sm" borderRadius={FIGMA.inputR} type="number" min={0}
                  value={row.kwhConsumed} onChange={e => updateRow(idx, "kwhConsumed", e.target.value)} placeholder="0" />
              </GridItem>
            ...
            {toNum(row.kwhConsumed) > 0 && (
              <Flex mt={3} p={2} bg={FIGMA.greenSoft} borderRadius="10px" align="center" justify="space-between">
                <Text fontSize="xs" color={FIGMA.green} fontWeight="600">
                  Facteur : {GRID_FACTORS[row.province] ?? 1.3} gCO₂e/kWh ({row.province})
                </Text>
                <Text fontSize="xs" color={FIGMA.green} fontWeight="bold">
                  {calcRow(row).tco2e.toFixed(7)} t CO₂e
                </Text>
              </Flex>
            )}
          </Box>
      ...
      {rows.length > 0 && (
        <Grid mt={6} templateColumns={{ base:"1fr", md:"repeat(3,1fr)" }} gap={3}>
          {[
            ...
            { label:"Total GES [tCO₂e]", val: totals.tco2e.toFixed(7) },
          ].map(card => (
...
export default Source4_1C1Form;
