...
export interface Source4_1E1FormProps {
  rows: Source4_1E1Row[];
  setRows: React.Dispatch<React.SetStateAction<Source4_1E1Row[]>>;
...
const SOURCE_CODE = "4.1E1";
const POSTE_NUM   = 4;

// kg CO₂e per cartridge unit
const CARTRIDGE_FACTORS: Record<string, number> = {
  "Cartouche d'encre standard (jet d'encre)":  0.045,
  "Cartouche d'encre grande capacité":           0.085,
  "Toner laser standard":                        0.550,
  "Toner laser grande capacité":                 1.200,
  "Toner laser très grande capacité":            2.100,
  "Ruban encreur (imprimante matricielle)":      0.030,
};
const CARTRIDGE_OPTIONS = Object.keys(CARTRIDGE_FACTORS);

const EMPTY_ROW: Source4_1E1Row = {
  description: "", date: "", site: "", product: "", reference: "",
  cartridgeType: CARTRIDGE_OPTIONS[2], qty: "",
};
...
const calcRow = (row: Source4_1E1Row) => {
  const qty    = toNum(row.qty);
  const factor = CARTRIDGE_FACTORS[row.cartridgeType] ?? 0.55;
  const kgCO2e = qty * factor;
  return { co2: kgCO2e * 1000, total: kgCO2e * 1000, tco2e: kgCO2e / 1000 };
};

export function Source4_1E1Form({ rows, setRows, posteSourceId, userId, bilanId, gesResults = [], setGesResults }: Source4_1E1FormProps) {
  ...
  const triggerSave = (next: Source4_1E1Row[]) => {
  ...
  const updateRow = (idx: number, key: keyof Source4_1E1Row, val: string) => {
    ...
    <Box bg={FIGMA.bg} borderRadius={FIGMA.r} p={{ base: 4, md: 6 }} boxShadow={FIGMA.cardShadow} animation={`${fadeInUp} 0.4s ease`}>
      <Flex justify="space-between" align="center" mb={5}>
        <VStack align="start" spacing={0}>
          <Heading size="md" color={FIGMA.green}>4.1E1 — Cartouches d&apos;encre et toner</Heading>
          <Text fontSize="sm" color={FIGMA.muted}>Émissions liées à la fabrication des cartouches d&apos;impression</Text>
        </VStack>
        <HStack spacing={2}>
            ...
            <Grid templateColumns={{ base: "1fr", md: "repeat(3,1fr)" }} gap={3}>
              <GridItem colSpan={{ base: 1, md: 3 }}>
                ...
                <Input size="sm" borderRadius={FIGMA.inputR} value={row.description}
                  onChange={e => updateRow(idx, "description", e.target.value)} placeholder="ex. Toner imprimante bureau RH…" />
              </GridItem>

              <GridItem colSpan={{ base: 1, md: 2 }}>
                <Text fontSize="xs" color={FIGMA.muted} mb={1}>Type de cartouche</Text>
                <Select size="sm" borderRadius={FIGMA.inputR} value={row.cartridgeType}
                  onChange={e => updateRow(idx, "cartridgeType", e.target.value)}>
                  {CARTRIDGE_OPTIONS.map(o => <option key={o}>{o}</option>)}
                </Select>
              </GridItem>
            ...
            {toNum(row.qty) > 0 && (
              <Flex mt={3} p={2} bg={FIGMA.greenSoft} borderRadius="10px" align="center" justify="space-between">
                <Text fontSize="xs" color={FIGMA.green} fontWeight="600">
                  Facteur : {CARTRIDGE_FACTORS[row.cartridgeType] ?? 0.55} kg CO₂e/cartouche
                </Text>
...
export default Source4_1E1Form;
