...
export interface Source4_3A1FormProps {
  rows: Source4_3A1Row[];
...
const SOURCE_CODE = "4.3A1";
const POSTE_NUM   = 4;

// kg CO₂e per m³ of wastewater treated
// Combines CH4 and N2O emission factors (IPCC 2006 / GHG Protocol)
const TREATMENT_FACTORS: Record<string, number> = {
  "Traitement aérobie (station municipale)":      0.360,
  "Traitement aérobie (fosse septique)":          0.120,
  "Traitement anaérobie (sans récupération CH₄)": 0.890,
  "Traitement anaérobie (avec récupération CH₄)": 0.180,
  "Bassin de stabilisation (lagunage)":           0.540,
  "Traitement primaire seulement":                0.600,
  "Rejet direct sans traitement":                 1.200,
};
const TREATMENT_OPTIONS = Object.keys(TREATMENT_FACTORS);

const EMPTY_ROW: Source4_3A1Row = {
  description: "", date: "", site: "", product: "", reference: "",
  treatmentType: TREATMENT_OPTIONS[0], volumeM3: "",
};
...
const calcRow = (row: Source4_3A1Row) => {
  const m3     = toNum(row.volumeM3);
  const factor = TREATMENT_FACTORS[row.treatmentType] ?? 0.36;
  const kgCO2e = m3 * factor;
...
export function Source4_3A1Form({ rows, setRows, posteSourceId, userId, bilanId, gesResults = [], setGesResults }: Source4_3A1FormProps) {
  ...
  const triggerSave = (next: Source4_3A1Row[]) => {
  ...
  const updateRow = (idx: number, key: keyof Source4_3A1Row, val: string) => {
    ...
    <Box bg={FIGMA.bg} borderRadius={FIGMA.r} p={{ base: 4, md: 6 }} boxShadow={FIGMA.cardShadow} animation={`${fadeInUp} 0.4s ease`}>
      <Flex justify="space-between" align="center" mb={5}>
        <VStack align="start" spacing={0}>
          <Heading size="md" color={FIGMA.green}>4.3A1 — Traitement des eaux usées</Heading>
          <Text fontSize="sm" color={FIGMA.muted}>Émissions CH₄ et N₂O liées au traitement des eaux usées</Text>
        </VStack>
        <HStack spacing={2}>
            ...
            <Grid templateColumns={{ base: "1fr", md: "repeat(3,1fr)" }} gap={3}>
              <GridItem colSpan={{ base: 1, md: 3 }}>
                ...
                <Input size="sm" borderRadius={FIGMA.inputR} value={row.description}
                  onChange={e => updateRow(idx, "description", e.target.value)} placeholder="ex. Eaux usées site de production…" />
              </GridItem>

              <GridItem colSpan={{ base: 1, md: 2 }}>
                <Text fontSize="xs" color={FIGMA.muted} mb={1}><Icon as={FiDroplet} mr={1} />Type de traitement</Text>
              ...
              <GridItem>
                <Text fontSize="xs" color={FIGMA.muted} mb={1}>Volume d&apos;eaux usées (m³)</Text>
                <Input size="sm" borderRadius={FIGMA.inputR} type="number" min={0}
                  value={row.volumeM3} onChange={e => updateRow(idx, "volumeM3", e.target.value)} placeholder="0" />
              </GridItem>
            ...
            {toNum(row.volumeM3) > 0 && (
              <Flex mt={3} p={2} bg={FIGMA.greenSoft} borderRadius="10px" align="center" justify="space-between">
                <Text fontSize="xs" color={FIGMA.green} fontWeight="600">
                  Facteur : {TREATMENT_FACTORS[row.treatmentType] ?? 0.36} kg CO₂e/m³ (CH₄ + N₂O)
                </Text>
...
export default Source4_3A1Form;
