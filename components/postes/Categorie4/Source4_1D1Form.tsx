...
export interface Source4_1D1FormProps {
  rows: Source4_1D1Row[];
  setRows: React.Dispatch<React.SetStateAction<Source4_1D1Row[]>>;
...
const SOURCE_CODE = "4.1D1";
const POSTE_NUM   = 4;

const PAPER_FACTORS: Record<string, number> = {
  "Papier standard 80g/m² (rame 500 feuilles)":  2.2,
  "Papier recyclé 80g/m² (rame 500 feuilles)":   1.3,
  "Papier photo 200g/m² (rame 500 feuilles)":     5.5,
  "Papier cartonné 160g/m² (rame 500 feuilles)":  4.4,
};
const PAPER_OPTIONS = Object.keys(PAPER_FACTORS);

const EMPTY_ROW: Source4_1D1Row = {
  description:"", date:"", site:"", product:"", reference:"", paperType:PAPER_OPTIONS[0], reamsQty:"",
};
...
const calcRow = (row: Source4_1D1Row) => {
  const reams  = toNum(row.reamsQty);
  const factor = PAPER_FACTORS[row.paperType] ?? 2.2;
  const kg     = reams * factor;
  return { co2: kg * 1000, total: kg * 1000, tco2e: kg / 1000 };
};

export function Source4_1D1Form({ rows, setRows, posteSourceId, userId, bilanId, gesResults=[], setGesResults }: Source4_1D1FormProps) {
  ...
  const triggerSave = (next: Source4_1D1Row[]) => {
  ...
  const updateRow = (idx: number, key: keyof Source4_1D1Row, val: string) => {
    ...
    <Box bg={FIGMA.bg} borderRadius={FIGMA.r} p={{ base:4, md:6 }} boxShadow={FIGMA.cardShadow} animation={`${fadeInUp} 0.4s ease`}>
      <Flex justify="space-between" align="center" mb={5}>
        <VStack align="start" spacing={0}>
          <Heading size="md" color={FIGMA.green}>4.1D1 — Papier d'imprimante</Heading>
          <Text fontSize="sm" color={FIGMA.muted}>Émissions liées à la production de papier utilisé</Text>
        </VStack>
        <HStack spacing={2}>
            ...
            <Grid templateColumns={{ base:"1fr", md:"repeat(3,1fr)" }} gap={3}>
              <GridItem colSpan={{ base:1, md:3 }}>
                ...
                <Input size="sm" borderRadius={FIGMA.inputR} value={row.description}
                  onChange={e => updateRow(idx, "description", e.target.value)} placeholder="ex. Papier imprimante bureau…" />
              </GridItem>

              <GridItem colSpan={{ base:1, md:2 }}>
                <Text fontSize="xs" color={FIGMA.muted} mb={1}>Type de papier</Text>
                <Select size="sm" borderRadius={FIGMA.inputR} value={row.paperType}
                  onChange={e => updateRow(idx, "paperType", e.target.value)}>
                  {PAPER_OPTIONS.map(p => <option key={p}>{p}</option>)}
                </Select>
              </GridItem>

              <GridItem>
                <Text fontSize="xs" color={FIGMA.muted} mb={1}>Nb de rames (500 feuilles)</Text>
                <Input size="sm" borderRadius={FIGMA.inputR} type="number" min={0}
                  value={row.reamsQty} onChange={e => updateRow(idx, "reamsQty", e.target.value)} placeholder="0" />
              </GridItem>
            ...
            {toNum(row.reamsQty) > 0 && (
              <Flex mt={3} p={2} bg={FIGMA.greenSoft} borderRadius="10px" align="center" justify="space-between">
                <Text fontSize="xs" color={FIGMA.green} fontWeight="600">
                  Facteur : {PAPER_FACTORS[row.paperType] ?? 2.2} kg CO₂e/rame
                </Text>
...
export default Source4_1D1Form;
