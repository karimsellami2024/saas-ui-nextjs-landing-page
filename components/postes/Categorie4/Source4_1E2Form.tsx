...
import { FiCalendar, FiMapPin, FiFileText, FiBattery } from "react-icons/fi";
import { supabase } from "../../../lib/supabaseClient";
import { ReferenceSelect } from '../ReferenceSelect';

export type Source4_1E2Row = {
  description: string;
  date: string;
  site: string;
  product: string;
...
export interface Source4_1E2FormProps {
  rows: Source4_1E2Row[];
  setRows: React.Dispatch<React.SetStateAction<Source4_1E2Row[]>>;
...
const SOURCE_CODE = "4.1E2";
const POSTE_NUM   = 4;

// kg CO₂e per unit
const BATTERY_FACTORS: Record<string, number> = {
  "Pile AA / AAA (alcaline)":                     0.014,
  "Pile 9V (alcaline)":                            0.025,
  "Bloc de piles C / D":                           0.040,
  "Batterie rechargeable Li-ion (petite — <20Wh)": 0.500,
  "Batterie rechargeable Li-ion (moyenne — 20-100Wh)": 2.000,
  "Batterie portable / banque d'alimentation":     3.500,
  "Batterie d'ordinateur portable":                5.000,
};
const BATTERY_OPTIONS = Object.keys(BATTERY_FACTORS);

const EMPTY_ROW: Source4_1E2Row = {
  description: "", date: "", site: "", product: "", reference: "",
  batteryType: BATTERY_OPTIONS[0], qty: "",
};
...
const calcRow = (row: Source4_1E2Row) => {
  const qty    = toNum(row.qty);
  const factor = BATTERY_FACTORS[row.batteryType] ?? 0.014;
  const kgCO2e = qty * factor;
  return { co2: kgCO2e * 1000, total: kgCO2e * 1000, tco2e: kgCO2e / 1000 };
};

export function Source4_1E2Form({ rows, setRows, posteSourceId, userId, bilanId, gesResults = [], setGesResults }: Source4_1E2FormProps) {
  ...
  const triggerSave = (next: Source4_1E2Row[]) => {
  ...
  const updateRow = (idx: number, key: keyof Source4_1E2Row, val: string) => {
    ...
    <Box bg={FIGMA.bg} borderRadius={FIGMA.r} p={{ base: 4, md: 6 }} boxShadow={FIGMA.cardShadow} animation={`${fadeInUp} 0.4s ease`}>
      <Flex justify="space-between" align="center" mb={5}>
        <VStack align="start" spacing={0}>
          <Heading size="md" color={FIGMA.green}>4.1E2 — Piles et batteries</Heading>
          <Text fontSize="sm" color={FIGMA.muted}>Émissions liées à la fabrication des piles et batteries utilisées</Text>
        </VStack>
        <HStack spacing={2}>
            ...
            <Grid templateColumns={{ base: "1fr", md: "repeat(3,1fr)" }} gap={3}>
              <GridItem colSpan={{ base: 1, md: 3 }}>
                ...
                <Input size="sm" borderRadius={FIGMA.inputR} value={row.description}
                  onChange={e => updateRow(idx, "description", e.target.value)} placeholder="ex. Piles télécommandes salles de réunion…" />
              </GridItem>

              <GridItem colSpan={{ base: 1, md: 2 }}>
                <Text fontSize="xs" color={FIGMA.muted} mb={1}><Icon as={FiBattery} mr={1} />Type de pile / batterie</Text>
                <Select size="sm" borderRadius={FIGMA.inputR} value={row.batteryType}
                  onChange={e => updateRow(idx, "batteryType", e.target.value)}>
                  {BATTERY_OPTIONS.map(o => <option key={o}>{o}</option>)}
                </Select>
              </GridItem>
            ...
            {toNum(row.qty) > 0 && (
              <Flex mt={3} p={2} bg={FIGMA.greenSoft} borderRadius="10px" align="center" justify="space-between">
                <Text fontSize="xs" color={FIGMA.green} fontWeight="600">
                  Facteur : {BATTERY_FACTORS[row.batteryType] ?? 0.014} kg CO₂e/unité
                </Text>
                <Text fontSize="xs" color={FIGMA.green} fontWeight="bold">
                  {calcRow(row).tco2e.toFixed(6)} t CO₂e
                </Text>
              </Flex>
            )}
          </Box>
...
export default Source4_1E2Form;
